        ...
        bool Database::recall_group_message(uint64_t message_id, uint64_t user_id) {
            ...
            sql << "UPDATE group_messages SET content = '[消息已撤回]', media_type = 0, media_url = '' "
                ...
                << "WHERE message_id = " << message_id;
                    ...
                    bool Database::save_fcm_token(uint64_t user_id, const std::string& fcm_token) {
                        ...
                        sql << "INSERT INTO user_fcm_tokens (user_id, fcm_token, updated_at) VALUES ("
                            ...
                            << user_id << ", '" << escape_string(fcm_token) << "', " << get_current_timestamp() << ") "
                            ...
                            << "ON DUPLICATE KEY UPDATE fcm_token = VALUES(fcm_token), updated_at = VALUES(updated_at)";
                        ...
                        if (mysql_query(connection_, sql.str().c_str()) != 0) {
                            ...
                            if (mysql_query(connection_, create_sql.c_str()) == 0) {
                    ...
                    std::string Database::get_fcm_token(uint64_t user_id) {
                        ...
                        sql << "SELECT fcm_token FROM user_fcm_tokens WHERE user_id = " << user_id;
                    ...
                    bool Database::remove_fcm_token(uint64_t user_id) {
                        ...
                        sql << "DELETE FROM user_fcm_tokens WHERE user_id = " << user_id;
